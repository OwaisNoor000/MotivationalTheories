...
import { getCriteriaReport, getSurveys } from "../hooks/requests";
import Menu from "../components/ui/Menu";
import MenuIcon from "../assets/menu-icon.svg";
import { Link } from "react-router-dom";
import WarningIcon from "../assets/warning.svg";
import { CriteriaReport } from "../types/report.types";
import BarChartIcon from "../assets/barchart-icon.svg";
...
export default function TheoryAnswerPage(){
    ...
    const [report,setReport] = useState<CriteriaReport[]>([]);
    ...
    useEffect(()=>{
        const request = async()=>{
            const reports:CriteriaReport[] = await getCriteriaReport();
            let surveyIdsResponse = [];
            for(const report of reports){
                surveyIdsResponse.push(report.surveyId);
            }
            setSurveyIds(surveyIdsResponse);
            setReport(reports)
                ...
                <div className={` w-[500px] h-fit bg-[#19B394] rounded-lg shadow-2xl/15 p-6`}>
                    <div className="font-bold text-3xl  flex flex-row justify-between cursor-pointer">
                        ...
                        <div className="flex flex-row items-center space-x-4">
                            <select onChange={(e)=>{setSelectedSurveyIndex(parseInt(e.target.value))}}
                            ...
                            <Link to={`/report/comparison/${surveyIds[selectedSurveyIndex]}`}>
                                <img src={BarChartIcon} className="h-10 w-10 border hover:scale-110 cursor-pointer"/>
                            </Link>
                        </div>
                        <div className="text-white mt-2">Answers:</div>
                        <select 
                        onChange={(e)=>{setSelectedAnswerId(parseInt(e.target.value))}}
                        className="px-10 py-5 bg-white shadow-xl text-gray-500 cursor-pointer hover:bg-gray-200 my-2 w-5/6">
                            { 
                                report.length!=0 &&
                                report[selectedSurveyIndex].answerIds.map((value,index)=>(
                                    <option value={index}>{value}</option>
                                ))
                            }
                        </select>
                </div>
                {report[selectedSurveyIndex] &&  !report[selectedSurveyIndex].eligible[theoryIndex] && 
                <div className="flex flex-row space-x-2 border rounded-sm p-2 text-lg border-red-500 text-red-500 m-4 ">
                    <img src={WarningIcon} className="h-8 w-8" />
                    {`${report[selectedSurveyIndex].ineligibleMessages[theoryIndex]}`}
                </div>
                }
                <div className="flex flex-row justify-center items-center space-x-2">
                        <Link to={report[selectedSurveyIndex]!=undefined?`/report/${surveyIds[selectedSurveyIndex]}/${report[selectedSurveyIndex].answerIds[selectedAnswerId]}/${selectedTheory}`:""}>
                        <button 
                        ...
                        <Link to={report[selectedSurveyIndex]!=undefined?`/report/record/${surveyIds[selectedSurveyIndex]}/${report[selectedSurveyIndex].answerIds[selectedAnswerId]}/${selectedTheory}`:""}>
                        <button 
